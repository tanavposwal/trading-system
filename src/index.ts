...
app.post("/order", (req: Request, res: Response) => {
  ...
  if (remainingQty === 0) {
    res.json({ msg: `number of quantity filled with orderbook are ${quantity}.` });
    return;
  }
  ...
  res.json({ msg: `number of quantity filled with orderbook are ${quantity - remainingQty}, numeber of quantity marked in orderbook are ${remainingQty}` })
})
...
app.get("/depth", (req: Request, res: Response) => {
  ...
  for (let j = 0; j < bids.length; j++) {
    const existingItemIndex = orderbook.bids.findIndex(item => item.price === bids[j].price);
  ...
  for (let j = 0; j < asks.length; j++) {
    const existingItemIndex = orderbook.asks.findIndex(item => item.price === asks[j].price);
